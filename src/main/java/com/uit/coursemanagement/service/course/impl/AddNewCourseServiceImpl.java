...
public class AddNewCourseServiceImpl extends AbstractBaseService<AddNewCourseRequest, Boolean>
    ...
    public void preExecute(AddNewCourseRequest addNewCourseRequest) {
        ...
        User user = userRepository.findById(addNewCourseRequest.getInstructor()).orElse(null);
        if (user == null){
        ...
        if (!user.getUserType().equals(EUserType.LECTURER)){
            throw new NotFoundException(messageHelper.getMessage(MessageCode.User.INVALID));
        }
    }
